...
namespace WindowsFormsApp1
{
    ...
    {
        private string strConn = @"DataSource=localhost; Database=F:\BKP\Particular\Irmãos Neto\Dados.FDB; User=sysdba; password=masterkey";
        private string strconSQL = "Server=exemplo-dns.eastus.cloudapp.azure.com,1433;Database=Exemplo-DB-01;User Id=exemplo-user-01;Password=123456789Ab@;";
        ...
        private void CriarConexaoSQL()
        {
           ...
           "(@nome, @Identificacao, @tipopessoa, @sexo, @ticketmedio, @ultimavenda, @valortotal, @valoritens, @qtItens, @datavenda, @idnf,@Veiculo, @Placa, @Ano, @Km,@FaixaAno, @FaixaTicketMedio)";
            ...
            string strcmdRevisaoAtrasada = @"INSERT INTO[dbo].[RevisaoAtrasada]
                                          ([NomeCliente]
        ...
        {
            StreamReader sr = new StreamReader(@"F:\BKP\Particular\Testes\nomes.csv");
        ...
        private void SaveCSV(DataTable dtSales)
        {
            ...
            //    string nome = row.ItemArray[1].ToString().Replace(';', ' ');
            //    int tipopessoa = row.ItemArray[4].ToString() == "F" ? 1 : 2;
            //    string vlrItens = Convert.ToDouble(row.ItemArray[5].ToString()).ToString().Replace(',', '.');
            //    string sexo = PegarSexo(nome, tipopessoa);
            //    sb.AppendFormat(rowFormat, nome, tipopessoa, vlrItens, sexo);
            //    sb.AppendLine();
            //}
            //File.WriteAllText(@"F:\BKP\Particular\Testes\vendas.csv", sb.ToString());
        }
